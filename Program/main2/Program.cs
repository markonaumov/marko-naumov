...
namespace main2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Jerry.FavoriteSongs = Songs.Where(x => x.Name.StartsWith("B")).ToList();
            
            //Jerry.GetFavoiteSongs();

            Maria.FavoriteSongs = Songs.Where(x => x.Length > 360).ToList();
            // Maria.GetFavoiteSongs();

            Jane.FavoriteSongs = Songs.Where(x => x.Genre == Genre.Rock).ToList();
            //Jane.GetFavoiteSongs();

            Stefan.FavoriteSongs = Songs.Where(x => x.Length < 180).Where(x => x.Genre == Genre.Rock).ToList();
            //Stefan.GetFavoiteSongs();

            FansArray = FansArray.Where(x => x.FavoriteSongs.Count >= 4).ToList();
